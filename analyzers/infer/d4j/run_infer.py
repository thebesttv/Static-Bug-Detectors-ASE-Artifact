...
_SANDBOX_DIR = abspath('/'.join(current_dir.split('/')[:-2]) + '/results/infer-proj-reports')
BUG_DIR = abspath('/'.join(current_dir.split('/')[:-2]) + '/results/infer-proj-files')
SCRIPT_FP = os.path.dirname(os.path.realpath(__file__))
_SOURCE_DIR = '/home/travis/build'

class InferRunner(ParallelArtifactRunner):
    ...
    def _get_command(bug_id, b_or_f, build_script):
        if build_script == 'build.xml':
            command = """export JAVA_HOME=/usr/lib/jvm/jdk1.8.0_221/jre
                ...
                /infer/infer/bin/infer --keep-going -- ant clean compile""".format(**{
            ...
            command = """export JAVA_HOME=/usr/lib/jvm/jdk1.8.0_221/jre
                export JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF8
                cd {source_dir}/{bug_id}{b_or_f}
                /infer/infer/bin/infer --keep-going -- mvn clean compile""".format(**{
                    'source_dir': _SOURCE_DIR,
                    'bug_id': bug_id,
                    'container_sandbox': _SANDBOX_DIR,
                    'b_or_f': b_or_f,
...
def _print_usage():
    print('Usage: python3 run_infer.py <bug_ids_file> <b_or_f>')
...
def main(argv=None):
    ...
    with open(bug_ids_file) as file:
        for line in file:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
                ti_start = time.time()
                for b_or_f in ['b', 'f']:
                    inf = InferRunner(b_or_f, [line], build_script, workers=1)
    ...
    t_end = time.time()
    print('Running Infer took {}s'.format(t_end-t_start))
